...
from itx_testkit.seeder.db.postgres_tables import Post, Subscription
    ...
    async def is_subscribed(self, subscriber_id: str, author_id: str) -> bool:
        async with self._session_factory() as session:
            stmt = select(Subscription).where(
                Subscription.subscriber_id == uuid.UUID(subscriber_id),
                Subscription.author_id == uuid.UUID(author_id),
            )
...
class PostgresDbSeeder(DbSeeder):
    ...
    def reader(self) -> PostgresDbReader:
        return PostgresDbReader(engine=self._engine)
